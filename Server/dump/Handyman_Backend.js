...
router.get('/user/:userId', async (req, res) => {
  try {
    ...
    // Exclude password from the response for security
    const { password, ...handymanDataWithoutPassword } = userData;
...
// Rename the current materials endpoints to invoice items
// 1. Get invoice items for a job (use existing materials endpoint)
// router.get('/bookings/:bookingId/materials', async (req, res) => {
//   try {
//     const { bookingId } = req.params;
//     console.log(`Getting invoice items for booking: ${bookingId}`);
    
//     if (!bookingId) {
//       return res.status(400).json({ error: 'Booking ID is required' });
//     }
    
//     // Get materials (now invoice items) from Firebase Realtime Database
//     const snapshot = await admin.database()
...
//         success: true,
//         materials: [] // Keep as "materials" for backward compatibility
//       });
//     }
    
//     const materialsData = snapshot.val();
//     console.log(`Found invoice items for booking: ${bookingId}`, materialsData);
    
//     // Convert to array format with IDs
//     const materials = Object.entries(materialsData).map(([id, data]) => ({
//       id,
//       ...data,
//     }));
...
//     res.status(200).json({ 
//       success: true,
//       materials: materials // Keep as "materials" for backward compatibility
//     });
...
//     res.status(500).json({ 
//       success: false,
//       error: 'Failed to get invoice items: ' + error.message 
//     });
//   }
// });

// 2. Add an invoice item to a job (update the existing materials endpoint)
// router.post('/bookings/:bookingId/materials', async (req, res) => {
//   try {
//     const { bookingId } = req.params;
//     const { name, quantity, price } = req.body;
    
//     console.log(`⚠️ Adding invoice item for booking: ${bookingId}`, { name, quantity, price });
    
//     if (!bookingId || !name || !quantity || !price) {
//       console.log('⚠️ Missing required data');
//       return res.status(400).json({ 
//         success: false,
//         error: 'Booking ID, name, quantity and price are required' 
//       });
//     }
    
//     // Create a new entry in the materials collection (used for invoice items)
//     const newItemRef = admin.database()
//       .ref(`materials/${bookingId}`)
//       .push();
    
//     console.log(`⚠️ Generated new invoice item ID: ${newItemRef.key}`);
...
//     console.log(`✅ Successfully added invoice item with ID: ${newItemRef.key}`);
    
//     // Also add a reference to the job
//     await admin.database()
//       .ref(`jobs/${bookingId}`)
//       .update({
//         hasMaterials: true,
//         hasInvoiceItems: true,  // Add this new field
//         updatedAt: new Date().toISOString()
//       });
...
//       // Update job with calculated amount only if total_fare is not already set
//       const jobSnapshot = await admin.database()
//         .ref(`jobs/${bookingId}`)
//         .once('value');
...
//           await admin.database()
//             .ref(`jobs/${bookingId}`)
//             .update({ total_fare: totalAmount });
          
//           console.log(`✅ Updated job ${bookingId} with calculated total fare: ${totalAmount}`);
//         }
//       }
//     }
...
// 3. Delete an invoice item (update the existing delete material endpoint)
// router.delete('/bookings/:bookingId/materials/:materialId', async (req, res) => {
//   try {
//     const { bookingId, materialId } = req.params;
    
//     console.log(`⚠️ Deleting invoice item: ${materialId} from booking: ${bookingId}`);
    
//     if (!bookingId || !materialId) {
//       return res.status(400).json({ 
//         success: false,
//         error: 'Booking ID and invoice item ID are required' 
//       });
//     }
    
//     // Check if the item exists before deleting
//     const itemSnapshot = await admin.database()
//       .ref(`materials/${bookingId}/${materialId}`)
//       .once('value');
      
//     if (!itemSnapshot.exists()) {
//       console.log(`❌ Invoice item not found: ${materialId}`);
//       return res.status(404).json({ 
//         success: false,
...
//     // Delete the material/invoice item
//     await admin.database()
//       .ref(`materials/${bookingId}/${materialId}`)
//       .remove();
    
//     console.log(`✅ Successfully deleted invoice item: ${materialId}`);
    
//     // Check if this was the last item for the job
//     const snapshot = await admin.database()
//       .ref(`materials/${bookingId}`)
//       .once('value');
    
//     if (!snapshot.exists()) {
//       // If no items left, update the job
//       await admin.database()
//         .ref(`jobs/${bookingId}`)
//         .update({
//           hasMaterials: false,
...
//       console.log(`✅ Updated job ${bookingId} with hasMaterials=false and hasInvoiceItems=false`);
//     } else {
...
//       // Update job with new calculated amount if total_fare was auto-calculated before
//       const jobSnapshot = await admin.database()
//         .ref(`jobs/${bookingId}`)
//         .once('value');
...
//       if (jobSnapshot.exists()) {
//         const jobData = jobSnapshot.val();
//         // Only auto-update if no manual override has been done
//         if (jobData.total_fare && !jobData.manual_fare) {
//           await admin.database()
//             .ref(`jobs/${bookingId}`)
//             .update({ total_fare: totalAmount });
          
//           console.log(`✅ Updated job ${bookingId} with recalculated total fare: ${totalAmount}`);
//         }
//       }
//     }
