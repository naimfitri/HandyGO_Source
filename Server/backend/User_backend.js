  ...
  try {
    ...
    if (snapshot.exists()) {
      snapshot.forEach((childSnapshot) => {
        ...
        // Skip cancelled or rejected jobs - they don't block the slot
        if (job.status === 'cancelled' || job.status === 'Rejected') {
          console.log(`Skipping ${job.status} job ${childSnapshot.key}`);
          return;
        }
        ...
        if (availability[jobDate]) {
          ...
          console.log(`Marked ${jobDate}, ${slotName} as unavailable due to existing job (status: ${job.status})`);
        }
      });
    }
  ...
  try {
    ...
    if (snapshot.exists()) {
      snapshot.forEach((childSnapshot) => {
        ...
        // Skip cancelled or rejected jobs - don't consider them booked
        if (job.status === 'cancelled' || job.status === 'Rejected') {
          console.log(`Skipping ${job.status} job ${childSnapshot.key}`);
          return;
        }
        ...
        if (!bookedSlots[dateString].includes(slotId)) {
          ...
          console.log(`Added booked slot: ${dateString}, ${slotName} (status: ${job.status})`);
        }
      });
...
router.post('/create-booking-with-fee', async (req, res) => {
  try {
    ...
    const { 
      ...
      starttimestamp, // Allow pre-calculated timestamps
      endtimestamp
    } = req.body;
...
router.post('/cancel-booking/:bookingId', async (req, res) => {
  try {
    ...
    // Find the original booking fee transaction to determine the exact refund amount
    const transactionsRef = db.ref('walletTransactions');
    ...
    if (transactionsSnapshot.exists()) {
      // Find the transaction with type "booking-fee" for this booking
      let bookingFeeTransaction = null;
      transactionsSnapshot.forEach(childSnapshot => {
        const transaction = childSnapshot.val();
        if (transaction.transactionType === 'booking-fee' && transaction.userId === user_id) {
          bookingFeeTransaction = transaction;
      ...
      if (bookingFeeTransaction) {
        // The amount is stored as negative, so we need to make it positive for refund
        processingFee = Math.abs(bookingFeeTransaction.amount);
        console.log(`Found original booking fee: ${processingFee} for booking ${bookingId}`);
      } else {
        // If no transaction found, fall back to the booking's processing_fee field
        // or global fare amount as before
        if (bookingData.processing_fee) {
          ...
          console.log(`No transaction found, using fallback amount: ${processingFee}`);
        }
      }
    } else {
      // No transactions found, use fallback
      console.log(`No transactions found for booking ${bookingId}, using fallback amount`);
      if (bookingData.processing_fee) {
        processingFee = parseFloat(bookingData.processing_fee);
      } else {
        // If not, get the global fare amount
...
router.post('/process-payment', async (req, res) => {
  try {
    ...
    // Validate required fields
    if (!userId || !bookingId || !handymanId || amount === undefined) {
      return res.status(400).json({ message: 'Missing required fields' });
    }
...
router.post('/users/:id/upload-image', upload.single('image'), async (req, res) => {
  try {
...
router.get('/images_handyman/:handymanId', async (req, res) => {
  try {
